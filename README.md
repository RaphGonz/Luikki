...
(`cloud/modal_app.py`). The app goes up with the artist's session (sign in from
Account), and the server checks it against Supabase's `luikki-supabase` secret;
the weights live in the `luikki-weights` Volume, T5 excluded.

    pip install -e ".[cloud]"
    modal run -m luikki.cloud.modal_app::download    # once
    modal deploy -m luikki.cloud.modal_app           # prints the URL

    LUIKKI_REMOTE_URL=<url> luikki serve --proposer remote
